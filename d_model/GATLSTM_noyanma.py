...
FEATURE_NAMES_PATH = train_cfg.get("FEATURE_NAMES_PATH") or train_cfg.get("FEATURE_NAME_PATH")
ADJ_PATH = train_cfg.get("ADJ_PATH")
BASE_OUTPUT_DIR = train_cfg.get("OUTPUT_DIR", "./results")
...
if PRED_LEN != 1:
    print(f"警告：当前代码为了兼容既有评价脚本，会强制使用单步输出。配置中的 PRED_LEN={PRED_LEN} 将被改为 1。")
    PRED_LEN = 1
...
print(f"SEQ_LEN={SEQ_LEN}, PRED_LEN={PRED_LEN}, BATCH_SIZE={BATCH_SIZE}")
...
print("实验目的: 保留历史水质输入，移除 mask 输入，检验模型是否利用了掩码信息")
...
input_feature_names = TIME_FEATURE_NAMES + meteo_names + target_names
...
print("\n========== 消融实验设置 ==========")
print("输入特征（时间+气象+历史水质，不含mask）:")
print(input_feature_names)
print("预测目标（水质）:")
print(target_names)
print("掩码（仅用于 loss 和评估，不作为输入特征）:")
print(mask_names)
print("=================================\n")
...
# 5. 构造滑动窗口
# =========================================================
def make_sequences(x, y, y_mask, seq_len, pred_len=1):
    """
    为了与第一段代码和既有评价脚本保持完全一致，这里固定做单步预测：
    X: (num_samples, seq_len, S, F_in)
    y: (num_samples, S, F_out)
    m: (num_samples, S, F_out)
    """
    if pred_len != 1:
        print(f"警告：make_sequences 收到 pred_len={pred_len}，但当前脚本固定按单步输出处理。")
...
X_train, Y_train, M_train = make_sequences(x_train, y_train, m_train_raw, SEQ_LEN, PRED_LEN)
X_val, Y_val, M_val = make_sequences(x_val, y_val, m_val_raw, SEQ_LEN, PRED_LEN)
X_test, Y_test, M_test = make_sequences(x_test, y_test, m_test_raw, SEQ_LEN, PRED_LEN)
