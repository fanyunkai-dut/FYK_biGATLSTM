...
cfg = config_all.get('GRIN_style_block_imputation_training', {})
...
class GRINTrainDataset(Dataset):
...
class GraphRITS(nn.Module):
    def __init__(self, input_dim, hidden_size, adj_norm, dropout=0.1):
        super().__init__()
    ...
    def forward(self, x, m, d, target_x=None, target_mask=None):
        ...
        for t in range(T):
            ...
            x_hist = self.hist_reg(h_graph)  # (B, N, F)
...
class GRINStyle(nn.Module):
    def __init__(self, input_dim, hidden_size, adj_norm, consistency_weight=0.1, dropout=0.1):
        super().__init__()
        self.fwd = GraphRITS(input_dim, hidden_size, adj_norm, dropout)
        self.bwd = GraphRITS(input_dim, hidden_size, adj_norm, dropout)
...
train_dataset = GRINTrainDataset(
    train_windows,
    TRAIN_BLOCK_LENGTHS,
...
val_dataset_dict = {
    L: GRINEvalDataset(val_windows, L, VAL_NUM_BLOCKS_PER_SAMPLE, VAL_BLOCK_SEED + i, USE_LOG1P_DT)
    for i, L in enumerate(VAL_BLOCK_LENGTHS)
}
test_dataset_dict = {
    L: GRINEvalDataset(test_windows, L, TEST_NUM_BLOCKS_PER_SAMPLE, TEST_BLOCK_SEED + i, USE_LOG1P_DT)
...
print("====== GRIN-style 人工大块遮挡 block-imputation 测试 ======")
...
with open(os.path.join(OUTPUT_DIR, 'Configs and results.txt'), 'w', encoding='utf-8') as f:
    f.write("========== GRIN-style Block Imputation Configuration ==========\n")
