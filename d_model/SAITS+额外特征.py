...
cfg = config_all.get('SAITS_exogenous_block_imputation_training', {})
...
def mask_single_station_window(values, masks, exo, block_lengths, num_blocks, rng):
    """
    ...
    return x_in, m_in, exo_in, target_x, target_mask


def build_station_windows(values, masks, exo, seq_len, stride):
    """
...
class SAITSExoTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        values, masks, exo = self.windows[idx]
        rng = np.random.default_rng(self.base_seed + self.epoch * 1000003 + idx)
        x_in, m_in, exo_in, target_x, target_mask = mask_single_station_window(
            values, masks, exo, self.block_lengths, self.num_blocks_per_sample, rng
        )
        return (
...
class SAITSExoEvalDataset(Dataset):
    def __init__(self, windows, block_length, num_blocks_per_sample=1, seed=123):
        self.samples = []
        for idx, (values, masks, exo) in enumerate(windows):
            rng = np.random.default_rng(seed + idx)
            x_in, m_in, exo_in, target_x, target_mask = mask_single_station_window(
                values, masks, exo, [block_length], num_blocks_per_sample, rng
            )
            self.samples.append((x_in, m_in, exo_in, target_x, target_mask))

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        x_in, m_in, exo_in, target_x, target_mask = self.samples[idx]
        return (
            torch.tensor(x_in, dtype=torch.float32),
            torch.tensor(m_in, dtype=torch.float32),
            torch.tensor(exo_in, dtype=torch.float32),
...
class SAITSExogenous(nn.Module):
    def __init__(self, input_dim, exo_dim, d_model=128, n_head=4, n_layers=2, d_ff=256, dropout=0.1, seq_len=192, exo_hidden=32):
        super().__init__()
    ...
    def forward(self, x, m, exo):
        # x, m: (B, T, F), exo: (B, T, F_exo)
...
train_windows = build_station_windows(train_values_std, train_masks_raw, train_exo_std, SEQ_LEN, WINDOW_STRIDE)
val_windows = build_station_windows(val_values_std, val_masks_raw, val_exo_std, SEQ_LEN, WINDOW_STRIDE)
test_windows = build_station_windows(test_values_std, test_masks_raw, test_exo_std, SEQ_LEN, WINDOW_STRIDE)
print(f"训练窗口数: {len(train_windows)}, 验证窗口数: {len(val_windows)}, 测试窗口数: {len(test_windows)}")

train_dataset = SAITSExoTrainDataset(train_windows, TRAIN_BLOCK_LENGTHS, TRAIN_NUM_BLOCKS_PER_SAMPLE, TRAIN_BLOCK_SEED)
val_dataset_dict = {L: SAITSExoEvalDataset(val_windows, L, VAL_NUM_BLOCKS_PER_SAMPLE, VAL_BLOCK_SEED + i)
                    for i, L in enumerate(VAL_BLOCK_LENGTHS)}
test_dataset_dict = {L: SAITSExoEvalDataset(test_windows, L, TEST_NUM_BLOCKS_PER_SAMPLE, TEST_BLOCK_SEED + i)
...
def run_epoch(loader, training=False, epoch=0):
    ...
    for x_in, m_in, exo_in, target_x, target_mask in loader:
        ...
        outputs = model(x_in, m_in, exo_in)
...
print("====== SAITS + exogenous 人工大块遮挡 block-imputation 测试 ======")
...
with open(os.path.join(OUTPUT_DIR, 'Configs and results.txt'), 'w', encoding='utf-8') as f:
    f.write("========== SAITS Exogenous Block Imputation Configuration ==========\n")
