...
# -*- coding: utf-8 -*-
"""
水质主分支 + 气象条件分支 + 上下游分离空间分支 WQ-Met-UpDown Context-SAITS

本脚本实现前三个分支：
- 强 SAITS 水质主分支
- 气象条件分支
- 上游/下游分离的方向感知空间分支
- 不使用 GAT，只用 upstream/downstream spatial cross-attention
...
# 上下游分离版本：不建议无邻居时 fallback 到所有站点。没有上游/下游时该方向上下文置 0。
SPATIAL_FALLBACK_TO_ALL = bool(cfg.get('SPATIAL_FALLBACK_TO_ALL', False))
# 方向模式：upstream / downstream / both。
SPATIAL_DIRECTION_MODE = str(cfg.get('SPATIAL_DIRECTION_MODE', 'both')).lower()
if SPATIAL_DIRECTION_MODE not in {'upstream', 'downstream', 'both'}:
    raise ValueError(f"SPATIAL_DIRECTION_MODE 必须是 upstream/downstream/both，当前为 {SPATIAL_DIRECTION_MODE}")
...
print(f'USE_SPATIAL_BRANCH: {USE_SPATIAL_BRANCH}, SPATIAL_N_LAYERS: {SPATIAL_N_LAYERS}, SPATIAL_CROSS_HEADS: {SPATIAL_CROSS_HEADS}, SPATIAL_DIRECTION_MODE: {SPATIAL_DIRECTION_MODE}')
...
def load_adjacency_matrix(path: str, expected_size: int) -> np.ndarray:
    ...
    # 当前矩阵约定：A_current[target, neighbor]=1 表示 target 可以参考 neighbor。
    # 你的表中 B3=1，即第3站可以参考第2站；因此 A_current 本身就是上游可参考矩阵。
    return mat


def build_up_down_adjacency(a_current: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    """
    根据当前输入邻接矩阵构造上游/下游两个方向矩阵。

    约定：后续 cross-attention 统一使用 adj[target, neighbor] = 1。

    你的当前表 A_current 已经是：
        A_current[target, upstream_neighbor] = 1
    所以：
        A_up   = A_current
        A_down = A_current.T

    两个方向都去掉对角线，避免空间分支重复看本站。
    """
    a_up = a_current.copy().astype(np.float32)
    a_down = a_current.T.copy().astype(np.float32)
    np.fill_diagonal(a_up, 0.0)
    np.fill_diagonal(a_down, 0.0)
    a_up = (a_up != 0).astype(np.float32)
    a_down = (a_down != 0).astype(np.float32)
    return a_up, a_down
...
class GatedMeteorologicalCrossAttention(nn.Module):
    """
    返回气象上下文 C_met，而不是直接改写 H_wq。

    后续在 ContextSAITS 中统一融合：
        H_enh = LN(H_wq + alpha_met*C_met + alpha_up*C_up + alpha_down*C_down)
    """
    def __init__(self):
        super().__init__()
...
class GatedSpatialCrossAttention(nn.Module):
    """
    返回空间上下文 C_sp，而不是直接改写 H_wq。

    adj 的约定固定为：adj[target_station, neighbor_station] = 1。
    - 上游分支传入 A_up
    - 下游分支传入 A_down
    没有该方向邻站时，返回 0 上下文。
    """
    def __init__(self, adj_matrix: np.ndarray, name: str = 'spatial'):
        super().__init__()
        self.name = name
    ...
    def _build_key_padding_mask(self, station_id: torch.Tensor, L: int) -> Tuple[torch.Tensor, torch.Tensor]:
        B = station_id.shape[0]
        S = self.adj.shape[0]
        allowed = self.adj[station_id.long()] > 0.0  # [B,S]
        if SPATIAL_EXCLUDE_SELF:
            row = torch.arange(B, device=station_id.device)
            allowed[row, station_id.long()] = False

        no_neighbor = allowed.sum(dim=1) == 0  # [B]
        safe_allowed = allowed.clone()
        if no_neighbor.any():
            # MultiheadAttention 不允许一整行 key 全部被 mask；临时允许自己作为 dummy key，后面将 ctx 置 0。
            rows = torch.where(no_neighbor)[0]
            safe_allowed[rows, station_id.long()[rows]] = True

        key_padding_mask = (~safe_allowed).unsqueeze(1).expand(B, L, S).reshape(B * L, S)
        return key_padding_mask, no_neighbor
    ...
    def forward(self, h_wq: torch.Tensor, h_all: Optional[torch.Tensor], station_id: torch.Tensor) -> torch.Tensor:
        ...
        key_padding_mask, no_neighbor = self._build_key_padding_mask(station_id, L)
        ctx, _ = self.attn(q, k, k, key_padding_mask=key_padding_mask, need_weights=False)
        ctx = ctx.reshape(B, L, D)
        if no_neighbor.any():
            ctx = ctx.masked_fill(no_neighbor.view(B, 1, 1), 0.0)
        g = self.gate(torch.cat([h_wq, ctx], dim=-1))
        return self.dropout(g * ctx)
...
class ContextSAITS(nn.Module):
    ...
    def __init__(
        ...
        num_stations: int,
        num_years: int,
        adj_up: Optional[np.ndarray] = None,
        adj_down: Optional[np.ndarray] = None,
    ):
        super().__init__()
        ...
        self.alpha_met_1 = nn.Parameter(torch.tensor(float(MET_ALPHA_INIT))) if self.met_encoder is not None else None
        self.alpha_met_2 = nn.Parameter(torch.tensor(float(MET_ALPHA_INIT))) if self.met_encoder is not None else None

        self.spatial_encoder = None
        self.spatial_up_cross_1 = None
        self.spatial_up_cross_2 = None
        self.spatial_down_cross_1 = None
        self.spatial_down_cross_2 = None
        self.alpha_up_1 = None
        self.alpha_up_2 = None
        self.alpha_down_1 = None
        self.alpha_down_2 = None
        if USE_SPATIAL_BRANCH:
            if adj_up is None and SPATIAL_DIRECTION_MODE in {'upstream', 'both'}:
                raise ValueError('USE_SPATIAL_BRANCH=True 且使用 upstream 时必须传入 adj_up')
            if adj_down is None and SPATIAL_DIRECTION_MODE in {'downstream', 'both'}:
                raise ValueError('USE_SPATIAL_BRANCH=True 且使用 downstream 时必须传入 adj_down')
            self.spatial_encoder = SpatialContextEncoder(input_dim, time_dim, static_dim, num_stations, num_years)
            if SPATIAL_DIRECTION_MODE in {'upstream', 'both'}:
                self.spatial_up_cross_1 = GatedSpatialCrossAttention(adj_up, name='upstream')
                self.spatial_up_cross_2 = GatedSpatialCrossAttention(adj_up, name='upstream')
                self.alpha_up_1 = nn.Parameter(torch.tensor(float(SPATIAL_ALPHA_INIT)))
                self.alpha_up_2 = nn.Parameter(torch.tensor(float(SPATIAL_ALPHA_INIT)))
            if SPATIAL_DIRECTION_MODE in {'downstream', 'both'}:
                self.spatial_down_cross_1 = GatedSpatialCrossAttention(adj_down, name='downstream')
                self.spatial_down_cross_2 = GatedSpatialCrossAttention(adj_down, name='downstream')
                self.alpha_down_1 = nn.Parameter(torch.tensor(float(SPATIAL_ALPHA_INIT)))
                self.alpha_down_2 = nn.Parameter(torch.tensor(float(SPATIAL_ALPHA_INIT)))

        self.enhance_norm_1 = nn.LayerNorm(D_MODEL)
        self.enhance_norm_2 = nn.LayerNorm(D_MODEL)
    ...
    def forward(
        ...
        if self.met_cross_1 is not None:
            enh1 = enh1 + self.alpha_met_1 * self.met_cross_1(h1, h_met)
        if self.spatial_up_cross_1 is not None:
            enh1 = enh1 + self.alpha_up_1 * self.spatial_up_cross_1(h1, h_spatial, station_id)
        if self.spatial_down_cross_1 is not None:
            enh1 = enh1 + self.alpha_down_1 * self.spatial_down_cross_1(h1, h_spatial, station_id)
        h1 = self.enhance_norm_1(enh1)
        ...
        if self.met_cross_2 is not None:
            enh2 = enh2 + self.alpha_met_2 * self.met_cross_2(h2, h_met)
        if self.spatial_up_cross_2 is not None:
            enh2 = enh2 + self.alpha_up_2 * self.spatial_up_cross_2(h2, h_spatial, station_id)
        if self.spatial_down_cross_2 is not None:
            enh2 = enh2 + self.alpha_down_2 * self.spatial_down_cross_2(h2, h_spatial, station_id)
        h2 = self.enhance_norm_2(enh2)
...
def main():
    ...
    adj_down = None
    if USE_SPATIAL_BRANCH:
        adj_current = load_adjacency_matrix(ADJ_PATH, expected_size=arrays.target_true_std.shape[1])
        adj_up, adj_down = build_up_down_adjacency(adj_current)
        print(f'[INFO] adjacency current shape: {adj_current.shape}, edge_count={int(adj_current.sum())}')
        print(f'[INFO] A_up=edge target->upstream count={int(adj_up.sum())} | A_down=edge target->downstream count={int(adj_down.sum())}')
        print('[INFO] 上下游约定：A_up = A_current, A_down = A_current.T，且二者对角线均置 0。')
    ...
    stats = {
        ...
        'adj_path': ADJ_PATH,
        'spatial_note': '上下游分离空间分支：A_up=A_current, A_down=A_current.T，二者对角线置0；目标站作为 Query，上游/下游邻站分别作为 Key/Value。',
        'spatial_direction_mode': SPATIAL_DIRECTION_MODE,
    ...
    for epoch in range(1, int(MAX_EPOCHS) + 1):
        ...
        row = {
            ...
            'alpha_met_stage1': float(model.alpha_met_1.detach().cpu()) if getattr(model, 'alpha_met_1', None) is not None else float('nan'),
            'alpha_met_stage2': float(model.alpha_met_2.detach().cpu()) if getattr(model, 'alpha_met_2', None) is not None else float('nan'),
            'alpha_up_stage1': float(model.alpha_up_1.detach().cpu()) if getattr(model, 'alpha_up_1', None) is not None else float('nan'),
            'alpha_up_stage2': float(model.alpha_up_2.detach().cpu()) if getattr(model, 'alpha_up_2', None) is not None else float('nan'),
            'alpha_down_stage1': float(model.alpha_down_1.detach().cpu()) if getattr(model, 'alpha_down_1', None) is not None else float('nan'),
            'alpha_down_stage2': float(model.alpha_down_2.detach().cpu()) if getattr(model, 'alpha_down_2', None) is not None else float('nan'),
        }
        history.append(row)
        ...
        print(
            ...
            f"alpha_up {row['alpha_up_stage1']:.3f}/{row['alpha_up_stage2']:.3f} | "
            f"alpha_down {row['alpha_down_stage1']:.3f}/{row['alpha_down_stage2']:.3f} | "
            f"lr {row['lr']:.2e}"
        )
