...
cfg = config_all.get('BRITS_exogenous_block_imputation_training', {})

SEQ_LEN = cfg.get('SEQ_LEN', 192)
WINDOW_STRIDE = cfg.get('WINDOW_STRIDE', 8)
...
def mask_single_station_window(values, masks, dts, exo, block_lengths, num_blocks, rng, use_log1p_dt=True):
    """
    values: (T, F_target) 标准化后的目标值
    masks:  (T, F_target) 自然观测掩码
    dts:    (T, F_target) 自然 dt (变换前)
    exo:    (T, F_exo)    完整外生特征（已标准化）

    返回：
        x_in:        (T, F_target) 人工遮挡后的输入值（缺失处填0）
        m_in:        (T, F_target) 人工遮挡后的输入 mask
        d_in:        (T, F_target) 人工遮挡后重新计算并变换后的 dt
        exo_in:      (T, F_exo)    外生特征，不做遮挡
...
def build_station_windows(values, masks, dts, exo, seq_len, stride):
    """
    把 (T, N, F) 切成 station-wise windows
    每个元素返回：
        values_win: (seq_len, F_target)
        masks_win:  (seq_len, F_target)
        dts_win:    (seq_len, F_target)
        exo_win:    (seq_len, F_exo)
    """
    T_local, N_local, F_target = values.shape
    _, _, F_exo = exo.shape
    windows = []
...
class BRITSExoTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        values, masks, dts, exo = self.windows[idx]
        rng = np.random.default_rng(self.base_seed + self.epoch * 1000003 + idx)
        x_in, m_in, d_in, exo_in, target_x, target_mask = mask_single_station_window(
            values, masks, dts, exo, self.block_lengths, self.num_blocks_per_sample, rng, self.use_log1p_dt
        )
        return (
...
class BRITSExoEvalDataset(Dataset):
    def __init__(self, windows, block_length, num_blocks_per_sample=1, seed=123, use_log1p_dt=True):
        self.samples = []
        for idx, (values, masks, dts, exo) in enumerate(windows):
            rng = np.random.default_rng(seed + idx)
            x_in, m_in, d_in, exo_in, target_x, target_mask = mask_single_station_window(
                values, masks, dts, exo, [block_length], num_blocks_per_sample, rng, use_log1p_dt
            )
...
class RITSExo(nn.Module):
    def __init__(self, input_dim, exo_dim, hidden_size, exo_hidden=32, dropout=0.1):
        super().__init__()
        ...
        self.rnn_cell = nn.LSTMCell(input_dim * 2 + exo_hidden, hidden_size)

    def forward(self, x, m, d, exo, target_x=None, target_mask=None):
        # x,m,d: (B, T, F_target); exo: (B, T, F_exo)
        B, T, F = x.size()
        ...
        for t in range(T):
            ...
            exo_t_proj = self.exo_proj(exo_t)
            inputs = torch.cat([c_c, m_t, exo_t_proj], dim=1)
...
class BRITSExo(nn.Module):
    def __init__(self, input_dim, exo_dim, hidden_size, exo_hidden=32, consistency_weight=0.1, dropout=0.1):
        super().__init__()
        self.rits_f = RITSExo(input_dim, exo_dim, hidden_size, exo_hidden, dropout)
        self.rits_b = RITSExo(input_dim, exo_dim, hidden_size, exo_hidden, dropout)
    ...
    def forward(self, x, m, d, exo, target_x=None, target_mask=None):
        imp_f, loss_f = self.rits_f(x, m, d, exo, target_x, target_mask)
        ...
        imp_b_rev, loss_b = self.rits_b(x_rev, m_rev, d_rev, exo_rev, target_x_rev, target_mask_rev)
...
exo_full = full_data[:, :, exo_idxs]        # (T, N, F_exo)
...
train_windows = build_station_windows(train_values_std, train_masks_raw, train_dts_proc, train_exo_std, SEQ_LEN, WINDOW_STRIDE)
val_windows = build_station_windows(val_values_std, val_masks_raw, val_dts_proc, val_exo_std, SEQ_LEN, WINDOW_STRIDE)
test_windows = build_station_windows(test_values_std, test_masks_raw, test_dts_proc, test_exo_std, SEQ_LEN, WINDOW_STRIDE)
print(f"训练窗口数: {len(train_windows)}, 验证窗口数: {len(val_windows)}, 测试窗口数: {len(test_windows)}")

train_dataset = BRITSExoTrainDataset(
    train_windows,
    TRAIN_BLOCK_LENGTHS,
...
val_dataset_dict = {
    L: BRITSExoEvalDataset(val_windows, L, VAL_NUM_BLOCKS_PER_SAMPLE, VAL_BLOCK_SEED + i, USE_LOG1P_DT)
    for i, L in enumerate(VAL_BLOCK_LENGTHS)
}
test_dataset_dict = {
    L: BRITSExoEvalDataset(test_windows, L, TEST_NUM_BLOCKS_PER_SAMPLE, TEST_BLOCK_SEED + i, USE_LOG1P_DT)
...
print("====== BRITS + exogenous 人工大块遮挡 block-imputation 测试 ======")
...
with open(os.path.join(OUTPUT_DIR, 'Configs and results.txt'), 'w', encoding='utf-8') as f:
    f.write("========== BRITS Exogenous Block Imputation Configuration ==========\n")
