...
# -*- coding: utf-8 -*-
"""
第一步：水质分支 WQ-SAITS Branch

本脚本只实现“水质分支”：
- 不使用 ADJ
- 不使用 GAT
- 不使用气象编码分支
- 只利用：单站点 5 个水质目标变量 + 输入 mask；不使用时间、静态、站点、gap embedding
...
cfg = config_all.get(
    ...
    config_all.get('WQ_SAITS_BRANCH_training', {}),
)
...
# baseline SAITS 不使用日历时间/静态/站点/gap embedding；只使用 [X_in, M_in]
TIME_FEATURE_NAMES = []
...
print('========== 当前纯 SAITS baseline 配置 ==========', flush=True)
...
def prepare_arrays() -> PreparedArrays:
    ...
    print('[INFO] time_dim:', time_raw.shape[-1], '| static_dim:', static_std.shape[-1])
...
class ContextSAITS(nn.Module):
    """
    纯 SAITS baseline：两阶段插补 + gate 融合 + 多重 loss。

    注意：
    - 不注入 year/month/day/hour 时间 embedding；
    - 不注入静态站点特征；
    - 不注入 station id embedding；
    - 不注入 gap embedding；
    - 主输入只使用 [x_in, m_in]，其中 x_in 是缺失位置填 0 后的 5 个水质变量，m_in 是输入 mask。

    这个版本适合作为 baseline，用来和后续的：
    SAITS + time/static/gap/station、SAITS + 气象、SAITS + 空间分支 做消融对比。
    """
    def __init__(
        self,
        input_dim: int,
    ...
    def forward(
        ...
        time_raw: Optional[torch.Tensor] = None,   # baseline 中忽略
        static: Optional[torch.Tensor] = None,     # baseline 中忽略
        gap: Optional[torch.Tensor] = None,        # baseline 中忽略
        station_id: Optional[torch.Tensor] = None, # baseline 中忽略
    ) -> Dict[str, torch.Tensor]:
        # stage 1：只用 [x_in, m_in]
        inp1 = torch.cat([x_in, m_in], dim=-1)
        base1 = self.input_proj_1(inp1)
        h1 = self.encoder_1(base1)
        x_tilde_1 = self.output_proj_1(h1)
        x_hat_1 = m_in * x_in + (1.0 - m_in) * x_tilde_1

        # stage 2：用第一次插补后的序列 + 原始输入 mask
        inp2 = torch.cat([x_hat_1, m_in], dim=-1)
        base2 = self.input_proj_2(inp2)
        h2 = self.encoder_2(base2)
...
def main():
    ...
    train_ds = WQWindowDataset(x_train, m_train, t_train, arrays.static_std, mode='train')
    val_ds = WQWindowDataset(x_val, m_val, t_val, arrays.static_std, mode='val')
    ...
    test_loader = None
    try:
        test_ds = WQWindowDataset(x_test, m_test, t_test, arrays.static_std, mode='test')
        if len(test_ds) > 0:
    ...
    stats = {
        ...
        'note': '预测值处于标准化空间；反标准化用 target_mean 和 target_std。',
    }
