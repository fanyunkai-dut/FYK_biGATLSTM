...
cfg = config_all.get('SAITS_exogenous_dynamic_bias_block_imputation_training', {})
...
def mask_single_station_window(values, masks, exo, station_idx, time_feats, block_lengths, num_blocks, rng):
    """
...
class SAITSExoDynamicBiasTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x_in, m_in, exo_in, station_idx, time_feats, target_x, target_mask = mask_single_station_window(
            values, masks, exo, station_idx, time_feats, self.block_lengths, self.num_blocks_per_sample, rng
        )
        return (
...
class SAITSExoDynamicBiasEvalDataset(Dataset):
    def __init__(self, windows, block_length, num_blocks_per_sample=1, seed=123):
        ...
        for idx, (values, masks, exo, station_idx, time_feats) in enumerate(windows):
            ...
            sample = mask_single_station_window(
                values, masks, exo, station_idx, time_feats, [block_length], num_blocks_per_sample, rng
            )
    ...
    def __getitem__(self, idx):
        x_in, m_in, exo_in, station_idx, time_feats, target_x, target_mask = self.samples[idx]
        return (
            torch.tensor(x_in, dtype=torch.float32),
            torch.tensor(m_in, dtype=torch.float32),
            torch.tensor(exo_in, dtype=torch.float32),
...
class SAITSExogenousDynamicBias(nn.Module):
    def __init__(
        self,
        input_dim,
        exo_dim,
    ...
    def forward(self, x, m, exo, station_idx, time_feats):
...
train_dataset = SAITSExoDynamicBiasTrainDataset(train_windows, TRAIN_BLOCK_LENGTHS, TRAIN_NUM_BLOCKS_PER_SAMPLE, TRAIN_BLOCK_SEED)
val_dataset_dict = {
    L: SAITSExoDynamicBiasEvalDataset(val_windows, L, VAL_NUM_BLOCKS_PER_SAMPLE, VAL_BLOCK_SEED + i)
    for i, L in enumerate(VAL_BLOCK_LENGTHS)
}
test_dataset_dict = {
    L: SAITSExoDynamicBiasEvalDataset(test_windows, L, TEST_NUM_BLOCKS_PER_SAMPLE, TEST_BLOCK_SEED + i)
...
# ==================== 模型训练 ====================
model = SAITSExogenousDynamicBias(
...
def run_epoch(loader, training=False, epoch=0):
    ...
    for x_in, m_in, exo_in, station_idx, time_feats, target_x, target_mask in loader:
        ...
        outputs = model(x_in, m_in, exo_in, station_idx, time_feats)
...
print("\n开始训练 SAITS + exogenous + dynamic hour-month / variable / station bias...")
...
print("====== SAITS + Exogenous + Dynamic Bias 人工大块遮挡 block-imputation 测试 ======")
...
with open(os.path.join(OUTPUT_DIR, 'Configs and results.txt'), 'w', encoding='utf-8') as f:
    f.write("========== SAITS Exogenous Dynamic Bias Block Imputation Configuration ==========\n")
