...
cfg = config_all.get('SDA_GRIN_style_block_imputation_training', {})
...
DYN_HEADS = cfg.get('DYN_HEADS', 4)
DYN_HIDDEN = cfg.get('DYN_HIDDEN', 64)
ADJ_FUSION_ALPHA = cfg.get('ADJ_FUSION_ALPHA', 0.5)
...
def build_graph_windows(values, masks, dts_raw, seq_len, stride):
...
def mask_single_station_graph_window(values, masks, dts_raw, block_lengths, num_blocks, rng, use_log1p_dt=True):
...
class SDAGRINTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        values, masks, dts_raw = self.windows[idx]
        rng = np.random.default_rng(self.base_seed + self.epoch * 1000003 + idx)
        x_in, m_in, d_in, target_x, target_mask = mask_single_station_graph_window(
            values, masks, dts_raw, self.block_lengths, self.num_blocks_per_sample, rng, self.use_log1p_dt
        )
        return (
...
class SDAGRINEvalDataset(Dataset):
    def __init__(self, windows, block_length, num_blocks_per_sample=1, seed=123, use_log1p_dt=True):
        self.samples = []
        for idx, (values, masks, dts_raw) in enumerate(windows):
            rng = np.random.default_rng(seed + idx)
            x_in, m_in, d_in, target_x, target_mask = mask_single_station_graph_window(
                values, masks, dts_raw, [block_length], num_blocks_per_sample, rng, use_log1p_dt
            )
...
class DynamicAdjacency(nn.Module):
    def __init__(self, node_feat_dim, dyn_hidden=64, heads=4, static_adj=None):
        super().__init__()
        self.node_feat_dim = node_feat_dim
        self.dyn_hidden = dyn_hidden
        self.heads = heads

        self.q_proj = nn.Linear(node_feat_dim, dyn_hidden * heads, bias=False)
        self.k_proj = nn.Linear(node_feat_dim, dyn_hidden * heads, bias=False)
        self.out_proj = nn.Linear(heads, 1, bias=False)
    ...
    def forward(self, node_repr):
        ...
        q = self.q_proj(node_repr).view(B, N, H, Dh).permute(0, 2, 1, 3)  # (B, H, N, Dh)
        k = self.k_proj(node_repr).view(B, N, H, Dh).permute(0, 2, 1, 3)  # (B, H, N, Dh)

        scores = torch.matmul(q, k.transpose(-2, -1)) / np.sqrt(Dh)        # (B, H, N, N)

        # 仅允许在静态邻接非零处 + 自环 传播
        static_mask = (self.static_adj > 0).unsqueeze(0).unsqueeze(0)       # (1,1,N,N)
        scores = scores.masked_fill(~static_mask, float('-inf'))

        attn = torch.softmax(scores, dim=-1)                                # (B,H,N,N)
        attn = attn.permute(0, 2, 3, 1)                                     # (B,N,N,H)
        attn = self.out_proj(attn).squeeze(-1)                              # (B,N,N)
        attn = torch.relu(attn)

        # 再做一遍按行归一化
        attn_sum = attn.sum(dim=-1, keepdim=True) + 1e-8
        dyn_adj = attn / attn_sum
        return dyn_adj
...
class DynamicGraphConv(nn.Module):
    ...
    def forward(self, x, adj_t):
        # x: (B,N,F), adj_t: (B,N,N)
        ax = torch.einsum('bij,bjf->bif', adj_t, x)
...
class DynamicGraphRITS(nn.Module):
    def __init__(self, input_dim, hidden_size, static_adj, static_adj_norm, dyn_heads=4, dyn_hidden=64, adj_fusion_alpha=0.5, dropout=0.1):
        super().__init__()
        self.input_dim = input_dim
        self.hidden_size = hidden_size
        self.adj_fusion_alpha = adj_fusion_alpha
        ...
        self.dynamic_adj_module = DynamicAdjacency(hidden_size, dyn_hidden=dyn_hidden, heads=dyn_heads, static_adj=static_adj)

        self.hist_graph = DynamicGraphConv(hidden_size, hidden_size)
        ...
        self.input_graph = DynamicGraphConv(input_dim, input_dim)
        self.dropout = nn.Dropout(dropout)
        self.rnn_cell = nn.GRUCell(input_dim * 3, hidden_size)

        self.register_buffer('static_adj_norm', torch.tensor(static_adj_norm, dtype=torch.float32))

    def blend_adj(self, dyn_adj):
        # dyn_adj: (B,N,N)
        static = self.static_adj_norm.unsqueeze(0).expand_as(dyn_adj)
        adj_t = (1.0 - self.adj_fusion_alpha) * static + self.adj_fusion_alpha * dyn_adj
        rowsum = adj_t.sum(dim=-1, keepdim=True) + 1e-8
        return adj_t / rowsum
    ...
    def forward(self, x, m, d, target_x=None, target_mask=None):
        ...
        for t in range(T):
            ...
            dyn_adj = self.dynamic_adj_module(h)
            adj_t = self.blend_adj(dyn_adj)

            h_graph = torch.relu(self.hist_graph(h, adj_t))
            ...
            x_graph = torch.relu(self.input_graph(c_c, adj_t))
            x_graph = self.dropout(x_graph)

            gru_in = torch.cat([c_c, m_t, x_graph], dim=-1).reshape(B * N, 3 * F)
...
class SDAGRINStyle(nn.Module):
    def __init__(self, input_dim, hidden_size, static_adj, static_adj_norm, dyn_heads=4, dyn_hidden=64, adj_fusion_alpha=0.5, consistency_weight=0.1, dropout=0.1):
        super().__init__()
        self.fwd = DynamicGraphRITS(input_dim, hidden_size, static_adj, static_adj_norm, dyn_heads, dyn_hidden, adj_fusion_alpha, dropout)
        self.bwd = DynamicGraphRITS(input_dim, hidden_size, static_adj, static_adj_norm, dyn_heads, dyn_hidden, adj_fusion_alpha, dropout)
    ...
    def forward(self, x, m, d, target_x=None, target_mask=None):
        imp_f, loss_f = self.fwd(x, m, d, target_x, target_mask)
        ...
        imp_b_rev, loss_b = self.bwd(x_rev, m_rev, d_rev, target_x_rev, target_mask_rev)
...
train_windows = build_graph_windows(train_values_std, train_masks_raw, train_dts_raw, SEQ_LEN, WINDOW_STRIDE)
val_windows = build_graph_windows(val_values_std, val_masks_raw, val_dts_raw, SEQ_LEN, WINDOW_STRIDE)
test_windows = build_graph_windows(test_values_std, test_masks_raw, test_dts_raw, SEQ_LEN, WINDOW_STRIDE)
print(f"训练窗口数: {len(train_windows)}, 验证窗口数: {len(val_windows)}, 测试窗口数: {len(test_windows)}")

train_dataset = SDAGRINTrainDataset(train_windows, TRAIN_BLOCK_LENGTHS, TRAIN_NUM_BLOCKS_PER_SAMPLE, TRAIN_BLOCK_SEED, USE_LOG1P_DT)
val_dataset_dict = {
    L: SDAGRINEvalDataset(val_windows, L, VAL_NUM_BLOCKS_PER_SAMPLE, VAL_BLOCK_SEED + i, USE_LOG1P_DT)
    for i, L in enumerate(VAL_BLOCK_LENGTHS)
}
test_dataset_dict = {
    L: SDAGRINEvalDataset(test_windows, L, TEST_NUM_BLOCKS_PER_SAMPLE, TEST_BLOCK_SEED + i, USE_LOG1P_DT)
...
model = SDAGRINStyle(
    ...
    static_adj=adj_raw,
    static_adj_norm=adj_norm,
    dyn_heads=DYN_HEADS,
    dyn_hidden=DYN_HIDDEN,
    adj_fusion_alpha=ADJ_FUSION_ALPHA,
...
print("====== SDA-GRIN-style 人工大块遮挡 block-imputation 测试 ======")
...
with open(os.path.join(OUTPUT_DIR, 'Configs and results.txt'), 'w', encoding='utf-8') as f:
    f.write("========== SDA-GRIN-style Block Imputation Configuration ==========\n")
    ...
    f.write(f"DYN_HEADS: {DYN_HEADS}\n")
    f.write(f"DYN_HIDDEN: {DYN_HIDDEN}\n")
    f.write(f"ADJ_FUSION_ALPHA: {ADJ_FUSION_ALPHA}\n")
