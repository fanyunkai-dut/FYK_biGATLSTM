...
if ADJ_PATH is None:
    raise ValueError('ADJ_PATH 不能为空')
...
def make_single_station_block_sample(values, masks, target_station, block_lengths, num_blocks, rng):
    t_local, _, f_dim = values.shape
...
class BaseStationwiseWindowDataset(Dataset):
    def __init__(self, values, masks, seq_len, sample_specs):
        self.values = values
        self.masks = masks
    ...
    def _get_window(self, idx):
        ...
        return values, masks, target_station


class GATSAITSTrainDataset(BaseStationwiseWindowDataset):
    def __init__(self, values, masks, seq_len, sample_specs, block_lengths, num_blocks_per_sample, seed=42):
        super().__init__(values, masks, seq_len, sample_specs)
    ...
    def __getitem__(self, idx):
        values, masks, target_station = self._get_window(idx)
        rng = np.random.default_rng(self.base_seed + self.epoch * 1000003 + idx)
        x_graph, m_graph, target_x, target_mask, target_station = make_single_station_block_sample(
            values, masks, target_station, self.block_lengths, self.num_blocks_per_sample, rng
        )
        return (
...
class GATSAITSEvalDataset(BaseStationwiseWindowDataset):
    def __init__(self, values, masks, seq_len, sample_specs, block_length, num_blocks_per_sample=1, seed=123):
        super().__init__(values, masks, seq_len, sample_specs)
    ...
    def __getitem__(self, idx):
        ...
        x_graph, m_graph, target_x, target_mask, target_station = make_single_station_block_sample(
            values, masks, target_station, [self.block_length], self.num_blocks_per_sample, rng
        )
        return (
            torch.from_numpy(x_graph),
            torch.from_numpy(m_graph),
...
class GATSAITS(nn.Module):
    def __init__(
        ...
        self.gat = MultiHeadGraphAttention(input_dim * 2, gat_hidden, num_heads=gat_heads, dropout=dropout)
        self.graph_dropout = nn.Dropout(dropout)

        total_in_dim = input_dim * 2 + gat_hidden
    ...
    def _spatial_encode(self, x_graph, m_graph):
        outs = []
        for t in range(x_graph.size(1)):
            node_in = torch.cat([x_graph[:, t], m_graph[:, t]], dim=-1)
    ...
    def forward(self, x_graph, m_graph, target_station):
        spatial_graph_h = self._spatial_encode(x_graph, m_graph)
        ...
        x_final = m_tgt * x_tgt + (1.0 - m_tgt) * x_comb
...
def gatsaits_loss(outputs, target_x, target_mask):
...
train_dataset = GATSAITSTrainDataset(
    train_values_std,
    train_masks_raw,
    SEQ_LEN,
    train_sample_specs,
...
test_dataset_dict = {
    ...
    for i, l_val in enumerate(TEST_BLOCK_LENGTHS)
}
...
test_loader_dict = {
    ...
    for l_val, ds in test_dataset_dict.items()
}
...
def run_epoch(loader, training=False, epoch=0):
    ...
    with context:
        for x_graph, m_graph, target_station, target_x, target_mask in loader:
            ...
            outputs = model(x_graph, m_graph, target_station)
            loss = gatsaits_loss(outputs, target_x, target_mask)
...
all_results = {}
for l_val, loader in test_loader_dict.items():
    metrics, pred, target, evalmask = evaluate_loader(loader)
    all_results[l_val] = metrics
    np.save(os.path.join(OUTPUT_DIR, f'block{l_val}_pred.npy'), pred)
    np.save(os.path.join(OUTPUT_DIR, f'block{l_val}_target.npy'), target)
    np.save(os.path.join(OUTPUT_DIR, f'block{l_val}_evalmask.npy'), evalmask)

print('====== GAT-SAITS 人工大块遮挡 block-imputation 测试 ======')
for l_val in sorted(all_results.keys()):
    metrics = all_results[l_val]
    if metrics is None:
        print(f'[block={l_val}] 没有可评估样本')
    else:
        print(
            f"[block={l_val}] R2={metrics['R2']:.4f}, RMSE={metrics['RMSE']:.4f}, "
            f"MAE={metrics['MAE']:.4f}, n_valid={metrics['n_valid']}"
        )

with open(os.path.join(OUTPUT_DIR, 'Configs and results.txt'), 'w', encoding='utf-8') as f:
    f.write('========== GAT-SAITS Block Imputation Configuration ==========\n')
    ...
    for l_val in sorted(all_results.keys()):
        metrics = all_results[l_val]
        if metrics is None:
            f.write(f'block={l_val}: no valid samples\n')
        else:
            f.write(
                f"block={l_val}: R2={metrics['R2']:.4f}, RMSE={metrics['RMSE']:.4f}, "
                f"MAE={metrics['MAE']:.4f}, n_valid={metrics['n_valid']}\n"
