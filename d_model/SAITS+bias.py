...
cfg = config_all.get('SAITS_dynamic_bias_block_imputation_training', {})
...
def mask_single_station_window(values, masks, station_idx, time_feats, block_lengths, num_blocks, rng):
    """
    ...
    返回：
        x_in:        (T, F_target) 人工遮挡后的输入值（缺失处填0）
        m_in:        (T, F_target) 人工遮挡后的输入 mask
        station_idx: int
        time_feats:  (T, F_time)
        target_x:    (T, F_target) 目标真实值（标准化）
        target_mask: (T, F_target) 仅在“人工遮挡且原本真实观测”的位置为1
    """
...
def build_station_windows(values, masks, time_feats_full, seq_len, stride):
    """
    把 (T, N, F) 切成 station-wise windows
    输出为 list，每个元素：
        values_win:    (seq_len, F_target)
        masks_win:     (seq_len, F_target)
        station_idx:   int
        time_feats:    (seq_len, F_time)
    """
...
class SAITSDynamicBiasTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        values, masks, station_idx, time_feats = self.windows[idx]
        rng = np.random.default_rng(self.base_seed + self.epoch * 1000003 + idx)
        x_in, m_in, station_idx, time_feats, target_x, target_mask = mask_single_station_window(
            values, masks, station_idx, time_feats, self.block_lengths, self.num_blocks_per_sample, rng
        )
        return (
...
class SAITSDynamicBiasEvalDataset(Dataset):
    def __init__(self, windows, block_length, num_blocks_per_sample=1, seed=123):
        self.samples = []
        for idx, (values, masks, station_idx, time_feats) in enumerate(windows):
            rng = np.random.default_rng(seed + idx)
            sample = mask_single_station_window(
                values, masks, station_idx, time_feats, [block_length], num_blocks_per_sample, rng
            )
    ...
    def __getitem__(self, idx):
        x_in, m_in, station_idx, time_feats, target_x, target_mask = self.samples[idx]
        return (
            torch.tensor(x_in, dtype=torch.float32),
            torch.tensor(m_in, dtype=torch.float32),
            torch.tensor(station_idx, dtype=torch.long),
...
class DynamicTVSBias(nn.Module):
    ...
    def forward(self, station_idx, time_feats):
        ...
        if self.use_station_bias:
            station_h = self.station_emb(station_idx)                      # (B, H)
            station_bias = self.station_proj(station_h).unsqueeze(1)       # (B, 1, F)
            bias = bias + station_bias

        if self.use_time_bias:
            time_h = self.time_encoder(time_feats)                         # (B, T, H)
            time_bias = self.time_proj(time_h)                             # (B, T, F)
...
class SAITSDynamicBias(nn.Module):
    def __init__(
        self,
        input_dim,
        num_stations,
    ...
    def forward(self, x, m, station_idx, time_feats):
...
def saits_dynamic_bias_loss(outputs, target_x, target_mask, bias_reg_weight=1e-4):
...
train_windows = build_station_windows(train_values_std, train_masks_raw, train_time_feats, SEQ_LEN, WINDOW_STRIDE)
val_windows = build_station_windows(val_values_std, val_masks_raw, val_time_feats, SEQ_LEN, WINDOW_STRIDE)
test_windows = build_station_windows(test_values_std, test_masks_raw, test_time_feats, SEQ_LEN, WINDOW_STRIDE)
print(f"训练窗口数: {len(train_windows)}, 验证窗口数: {len(val_windows)}, 测试窗口数: {len(test_windows)}")

train_dataset = SAITSDynamicBiasTrainDataset(train_windows, TRAIN_BLOCK_LENGTHS, TRAIN_NUM_BLOCKS_PER_SAMPLE, TRAIN_BLOCK_SEED)
val_dataset_dict = {
    L: SAITSDynamicBiasEvalDataset(val_windows, L, VAL_NUM_BLOCKS_PER_SAMPLE, VAL_BLOCK_SEED + i)
    for i, L in enumerate(VAL_BLOCK_LENGTHS)
}
test_dataset_dict = {
    L: SAITSDynamicBiasEvalDataset(test_windows, L, TEST_NUM_BLOCKS_PER_SAMPLE, TEST_BLOCK_SEED + i)
...
def run_epoch(loader, training=False, epoch=0):
    ...
    for x_in, m_in, station_idx, time_feats, target_x, target_mask in loader:
        ...
        outputs = model(x_in, m_in, station_idx, time_feats)
        loss = saits_dynamic_bias_loss(outputs, target_x, target_mask, bias_reg_weight=BIAS_REG_WEIGHT)
...
print("\n开始训练 SAITS + dynamic hour-month / variable / station bias...")
...
def evaluate_loader(loader):
    model.eval()
    all_pred, all_target, all_evalmask, all_bias = [], [], [], []
    with torch.no_grad():
        for x_in, m_in, station_idx, time_feats, target_x, target_mask in loader:
            x_in = x_in.to(device, non_blocking=True)
            m_in = m_in.to(device, non_blocking=True)
    ...
    return metrics, pred, target, evalmask, bias
...
for L, loader in test_loader_dict.items():
    metrics, pred, target, evalmask, bias = evaluate_loader(loader)
...
print("====== SAITS + Dynamic Bias 人工大块遮挡 block-imputation 测试 ======")
...
with open(os.path.join(OUTPUT_DIR, 'Configs and results.txt'), 'w', encoding='utf-8') as f:
    f.write("========== SAITS Dynamic Bias Block Imputation Configuration ==========\n")
