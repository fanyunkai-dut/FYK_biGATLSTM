#气象数据处理脚本（降水）：1.转换UTC时间到北京时间 2.一天8测变成一天6测，没有的时间点做线性插值 3.根据excel表格里的经纬度去做某子流域内所有气象网格的降雨量加和
import os
import re
import numpy as np
import pandas as pd
...
preprocess_cfg = config_all['QX_precidata_preprocessing']

# 生成文件的北京时间范围（从 YAML 读取为字符串）
START_BEIJING_str = preprocess_cfg['start_time']
END_BEIJING_str   = preprocess_cfg['end_time']
nc_folder      = preprocess_cfg['nc_path']
excel_file     = preprocess_cfg['lon_lat_path']
output_file    = preprocess_cfg['output']

# 将字符串转换为 datetime 对象（pandas Timestamp 亦可，但后续用 datetime）
START_BEIJING = pd.to_datetime(START_BEIJING_str).to_pydatetime()
END_BEIJING   = pd.to_datetime(END_BEIJING_str).to_pydatetime()
# =================================================

# 以下函数定义保持不变（parse_filename, get_all_nc_files, read_excel_stations, 
# build_grid_kdtree, find_nearest_indices, read_precip_data）
...
def read_precip_data(file_path, var_name='precipitation', fill_value=-9999.0):
    ds = nc.Dataset(file_path)
    precip = ds.variables[var_name][0, :, :]  # (lat, lon)
    if hasattr(ds.variables[var_name], '_FillValue'):
        fill = ds.variables[var_name]._FillValue
    else:
        fill = fill_value
    precip = np.where(precip == fill, 0.0, precip)
    ds.close()
    return precip.flatten()
...
def main():
    ...
    # 4. 生成所有输出时刻（北京时间），直接用 pandas 生成每4小时间隔
    output_times_beijing = pd.date_range(start=START_BEIJING, end=END_BEIJING, freq='4H').to_pydatetime().tolist()
    ...
    for i, beijing_dt in enumerate(output_times_beijing):
        ...
        if hour_beijing == 0:
            ...
            if utc1 not in time_to_file or utc2 not in time_to_file:
                ...
                continue
            p1 = read_precip_data(time_to_file[utc1])
            p2 = read_precip_data(time_to_file[utc2])
            for s_idx, indices in enumerate(station_indices):
                val = np.sum(w1 * p1[indices] + w2 * p2[indices])
                result[i, s_idx] = val
            ...
            if utc not in time_to_file:
                ...
                continue
            precip_flat = read_precip_data(time_to_file[utc])
            for s_idx, indices in enumerate(station_indices):
                result[i, s_idx] = np.sum(precip_flat[indices])
