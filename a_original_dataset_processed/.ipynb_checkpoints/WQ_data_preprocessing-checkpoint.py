#处理水质数据集：1.删除早于2020-11-09的数据；2.补全缺失的时间点（每天00,04,08,12,16,20）；3.删除指定列；4.处理异常值和滑动窗口处理。
import os
import pandas as pd
import numpy as np

# 第一步：删除早于2020-11-09的数据
def filter_by_date(df, start_date="2020-11-09 00:00:00"):
    df['监测时间'] = pd.to_datetime(df['监测时间'], format='%Y-%m-%d %H:%M:%S')
    df = df[df['监测时间'] >= pd.to_datetime(start_date)]
    return df

# 补全缺失的时间点（每天00,04,08,12,16,20）
def add_missing_timepoints(df):
    """
    确保从最早日期到最晚日期，每天都有 00:00, 04:00, 08:00, 12:00, 16:00, 20:00 六个时刻。
    缺失的时刻会被补全，对应指标列为NaN。
    """
    # 获取最早日期当天的00:00和最晚日期当天的20:00
    start_date = df['监测时间'].min().normalize()                     # 最早日期的 00:00
    end_date = df['监测时间'].max().normalize() + pd.DateOffset(hours=20)  # 最晚日期的 20:00
    # 生成完整的时间序列，间隔4小时
    full_time = pd.date_range(start=start_date, end=end_date, freq='4H')
    full_df = pd.DataFrame({'监测时间': full_time})
    # 左连接，以完整时间序列为基准，保留所有时刻
...
def process_files(input_folder, output_folder):
    ...
    for file in files:
        ...
        # 检查过滤后是否为空
        if df.empty:
            msg = f"文件 {file} 在删除早于2020-11-09的数据后为空，跳过处理。"
            print(msg)
...
# 执行数据处理
input_folder = "/home/fanyunkai/FYK_GCNLSTM/dataset/WQ_original_dataset/"  # 输入文件夹路径
output_folder = "/home/fanyunkai/FYK_GCNLSTM/dataset/WQ_processed_dataset/"  # 输出文件夹路径
