...
def generate_time_features(time_index):
    """
    为“可训练时间 embedding”生成类别编号，而不是直接生成普通数值特征。

    注意：
        这里输出的 year_id / month_id / day_id / hour_id 只是 embedding 的输入索引。
        后续模型里需要用 nn.Embedding 把这些索引变成时间向量。
        这 4 个字段不要做标准化、归一化，也不要当普通连续变量直接喂给模型。

    返回：
        time_features: (T, 4), float32 保存；进模型前需要 .long()
        time_feature_names: ['year_id', 'month_id', 'day_id', 'hour_id']
    """
    dates = pd.DatetimeIndex(time_index)

    # 年份从 0 开始编号。例如数据从 2021 年开始：
    # 2021 -> 0, 2022 -> 1, 2023 -> 2, 2024 -> 3
    year_id = (dates.year - dates.year[0]).astype(np.int64)

    # 月份、日期转成从 0 开始的类别编号，方便 nn.Embedding 使用
    month_id = (dates.month - 1).astype(np.int64)  # 1-12 -> 0-11
    day_id = (dates.day - 1).astype(np.int64)      # 1-31 -> 0-30

    # 小时本身就是 0-23
    # 如果你的数据固定 4 小时一步，也可以改成 hour_slot_id = dates.hour // 4，范围是 0-5
    hour_id = dates.hour.astype(np.int64)
